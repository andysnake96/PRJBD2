...
----------+----------+----------+-----------+----------------------+------------------+-------------------+------------------
 -1.56554 |  4.95194 |  1.49857 | 355.04486 | -0.00288061079447488 | 182.427816483319 | 0.385024071407913 | 22802.4189530986
    ...
    private final double brightness=0.2;
    ...
    void searchFilamentByBrightnessAndEllipticity() throws SQLException {
       List<Filament> out = ExtendedSearchFilamentDAO.searchFilamentByBrightnessAndEllipticity(brightness,ellipticityRange);
        boolean filamentDebugContained=false;
        boolean filamentDebugUnContained=false;
       for(int z=0;z<out.size();z++){
           if ( out.get(z).getInstrument().getName().equals(brightnesNEllipticityCheck.getInstrument().getName()) &&
                   (out.get(z).getId() == brightnesNEllipticityCheck.getId()))
               filamentDebugContained=true;

           if ( out.get(z).getInstrument().getName().equals(brightnesNEllipticityUnContainedCheck.getInstrument().getName()) &&
    ...
    void searchFilamentByCircle() throws SQLException {
         ...
         */
        List<Filament> out= ExtendedSearchFilamentDAO.searchFilamentByCircle(glat,glon,radius);
        boolean filamentDebugContained=false;
        for(int z=0;z<out.size();z++){
            if ( out.get(z).getInstrument().getName().equals(filamentInCircle.getInstrument().getName()) &&
    ...
    void searchFilamentBySquare() throws SQLException {
         ...
         */
        List<Filament> out= ExtendedSearchFilamentDAO.searchFilamentByRectangle(glat,glon,side,side);
        boolean filamentDebugContained=false;
        for(int z=0;z<out.size();z++){
            if ( out.get(z).getInstrument().getName().equals(filamentInSquare.getInstrument().getName()) &&
